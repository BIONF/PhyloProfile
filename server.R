...
shinyServer(function(input, output, session) {
        ...
        for (i in 1:nrow(taxaNameDf)){
          ...
          if (is.na(id)){
            ...
            if (nrow(temp) > 0){
              ...
              if (is.na(new_id)){
                idDf[i, ] <- c(as.character(taxaNameDf[i, ]),
          ...
          incProgress(1 / nrow(taxaNameDf),
                      detail = paste(i, "/", nrow(taxaNameDf)))
        }
      })
  ...
  output$var1_id.ui <- renderUI({
    ...
    if(input$demo_data == "ampk-tor"){
      ...
      # get var1/var2 names based on input 
      # (only if input file in long format table)
      if(is.null(filein)){
        return(textInput("var1_id",
                         h5("1st variable:"),
                         value = "Variable 1",
                         width = "100%",
                         placeholder = "Name of first variable"))
  ...
  output$var2_id.ui <- renderUI({
    # if(input$demo == TRUE){
       # textInput("var2_id", h5("Second variable:"),
       #           value = "Traceability",
       #           width = "100%",
       #           placeholder = "Name of second variable")
    if(input$demo_data == "ampk-tor"){
      textInput("var2_id", h5("2nd variable:"),
                value = "Domain similarity (backward)",
                width = "100%", 
                placeholder = "Name of second variable")
    } else if(input$demo_data == "demo"){
      textInput("var2_id", h5("2nd variable:"),
      ...
      # (only if input file in long format table)
      if(is.null(filein)){
        return(textInput("var2_id", h5("2nd variable:"),
                         value = "Variable 2",
                         width = "100%",
                         placeholder = "Name of second variable"))
        }    
      ...
      # # OMA -------------------------------------------------------------------
      # } else if (input_type == "oma") {
      #   # Add dependency on the get data button 
      ...
      #   isolate({
      #     print("isolate")
      #     longDf <- oma_parser (filein$datapath, input$selected_oma_type)
      #     textInput("var2_id", h5("2nd variable:"),
      #               value = colnames(longDf)[5],
      #               width = "100%",
      #               placeholder = "Name of second variable")
  ...
  output$checkNewick.ui <- renderUI({
    ...
    checkNewick = checkNewick(filein, input$demo_data, input$mainInput)
    if(checkNewick == 1){
  ...
  output$unk_taxa_status <- reactive({
    unkTaxa <- unkTaxa(input$demo_data, input$mainInput)
    length(unkTaxa) > 0
  })
  ...
  output$unk_taxa_full <- renderDataTable(option = list(searching = FALSE,
                                                        pageLength = 10),{
    if(length(unkTaxa(input$demo_data, input$mainInput)) > 0){
      tb <- as.data.frame(unkTaxa(input$demo_data, input$mainInput))
  ...
  output$select = renderUI({
    choice <- alltaxa_list(input$mainInput,
                           input$demo_data,
                           input$rank_select)
  ...
  observe({
    ...
    if(!is.null(input$plot_dblclick)){
      ...
      dataHeat <- dataHeat(input$percent,
                           input$var1,
                           input$var2,
                           input$mainInput,
                           input$demo_data,
                           input$var1_relation,
                           input$var2_relation,
                           input$var1_aggregate_by, input$var2_aggregate_by,
                           input$end_index,
                           input$gene_list_selected,
                           input$list,
                           input$st_index,
                           input$ordering,
                           input$gene_list_selected,
      ...
      updateSelectInput(session, "taxonHighlight",
  ...
  output$highlight_gene_ui = renderUI({
    geneList <- dataHeat(input$percent,
                         input$var1,
                         input$var2,
                         input$mainInput,
                         input$demo_data,
  ...
  observe({
    if(!is.null(input$plot_dblclick)){
      ...
      clickedInfo <- mainpoint_info(v, input$rank_select, input$in_select, input$percent,
                                    input$var1,
                                    input$var2,
                                    input$mainInput,
                                    input$demo_data,
      ...
      updateSelectInput(session, "geneHighlight",
                        label = "Highlight:",
                        choices = out,
                        selected = out[corY+1])
    } else return()
  ...
  output$total_gene_number.ui <- renderUI({
    geneList <- preData(input$end_index,
                        input$gene_list_selected,
                        input$list,
                        input$st_index,
                        input$demo_data,
  ...
  get_sortedtaxa_list <- reactive({
    if(v$doPlot == FALSE) return() 
    
    # Get representative taxon 
    # load list of unsorted taxa
    Dt <- get_taxa_list(FALSE, demo_data, main_input)
    ...
    taxa_list <- as.data.frame(read.table("data/taxonNamesReduced.txt",
                                          sep = "\t",
                                          header = T))
    allTaxa <- alltaxa_list(main_input, demo_data, rank_select)
    ...
    print("abbrName")
    print(as.character(repTaxon$abbrName))
    if(is.null(treeIn)){
    ...
    inputTaxa <- subset_taxa(demo_data, main_input)
  ...
  sortedtaxa_list <- get_sortedtaxa_list
  ...
  output$gene_in = renderUI({
    ...
    if(v$doPlot == FALSE){return(selectInput("inSeq", "", "all"))}
      ...
      data <- as.data.frame(get_data_filtered(input$var1_aggregate_by, input$var2_aggregate_by,
                                              input$end_index,
                                              input$selected,
                                              input$listIn,
                                              input$st_index,
      ...
      if (input$add_custom_profile == TRUE){
        ...
        out <- significant_genes_gc$geneID
        if(length(out)>0){
          selectInput("inSeq", "",
                      out,
                      selected = as.list(out),
  ...
  output$plot_download <- downloadHandler(
    ...
    content = function(file) {
      ggsave(file, plot = mainPlot(input$apply_cluster,
                                   input$x_axis,
                                   input$var1_id,
                                   input$var2_id,
                                   input$low_color_var1,
  ...
  distDf <- reactive({
    ...
    if(input$demo_data == "demo" | input$demo_data == "ampk-tor"){
      ...
      # LONG ------------------------------------------------------------------
        ...
        dataOrig <- as.data.frame(read.table(file = filein$datapath,
                                             sep = "\t",
                                             header = T,
                                             check.names = FALSE,
                                             comment.char = ""))
      ...
      # WIDE ------------------------------------------------------------------
        ...
        # replace NA value with "NA#NA" 
        # (otherwise the corresponding orthoID will be empty)
    ...
    if(input$dataset.distribution == "Customized data"){
      ...
      allData <- get_data_filtered(input$var1_aggregate_by, input$var2_aggregate_by,
                                   input$end_index,
                                   input$selected,
                                   input$listIn,
                                   input$st_index,
  ...
  output$var1DistPlot <- renderPlot(width = 512, height = 356,{
    if(input$auto_update == FALSE){
      ...
      # is changed
      isolate({
        var1DistPlot(input$dist_text_size, input$var1_id, v)
      })
    } else {
      var1DistPlot(input$dist_text_size, input$var1_id, v)
    }
  ...
  output$var2DistPlot <- renderPlot(width = 512, height = 356,{
    if(input$auto_update == FALSE){
      ...
      # is changed
      isolate({
        var2DistPlot(input$dist_text_size, input$var2_id, v)
      })
    } else {
      var2DistPlot(input$dist_text_size, input$var2_id, v)
    }
  ...
  presSpecAllDt <- reactive({
    ...
    if(input$demo_data == "demo" | input$demo_data == "ampk-tor"){
      if(input$demo_data == "demo"){
        mdData <- read.table("https://raw.githubusercontent.com/BIONF/phyloprofile-data/master/demo/test.main.long",
                             sep = "\t",
                             header = T,
                             fill = T,
                             stringsAsFactors = FALSE)
      } else {
        mdData <- read.table("https://raw.githubusercontent.com/BIONF/phyloprofile-data/master/expTestData/ampk-tor/ampk-tor.phyloprofile",
                             sep = "\t",
                             header = T,
                             fill = T,
                             stringsAsFactors = FALSE)
      ...
      # LONG ------------------------------------------------------------------
        ...
        mdData <- as.data.frame(read.table(file = filein$datapath,
                                           sep = "\t",
                                           header = T,
                                           check.names = FALSE,
                                           comment.char = ""))
      ...
      # WIDE ------------------------------------------------------------------
        ...
        data <- as.data.frame(read.table(file = filein$datapath,
                                         sep = "\t",
                                         header = T,
                                         check.names = FALSE,
                                         comment.char = ""))
  ...
  output$presSpecPlot <- renderPlot(width = 512, height = 356,{
    if(input$auto_update == FALSE){
      ...
      # is changed
      isolate({
        presSpecPlot(input$percent, input$dist_text_size, v)
      })
    } else {
      presSpecPlot(input$percent, input$dist_text_size, v)
    }
  ...
  output$plot_download_dist <- downloadHandler(
    ...
    content = function(file) {
      if(input$selected_dist == input$var1_id){
        ggsave(file,
               plot = var1DistPlot(input$dist_text_size, input$var1_id, v ),
               dpi=300, device = "pdf", limitsize=FALSE)
      }
      if(input$selected_dist == input$var2_id){
        ggsave(file,
               plot = var2DistPlot(input$dist_text_size, input$var2_id, v),
               dpi=300, device = "pdf", limitsize=FALSE)
      }
      if(input$selected_dist == "% present taxa"){
        ggsave(file, plot = presSpecPlot(input$percent, input$dist_text_size, v), dpi=300, device = "pdf", limitsize=FALSE)
      }
    }
  )
  ...
  # change label of plotCustom button -----------------------------------------
  # if auto_update_selected is unchecked 
  output$plot_custom_btn <- renderUI({
    if(input$auto_update_selected == FALSE){
      shinyBS::bsButton("plotCustom",
  ...
  taxa_select_cus <- reactive({
    ...
    if(length(rank_select_cus) == 0){return()}
      ...
      ### load list of unsorted taxa
      Dt <- get_taxa_list(TRUE, input$demo_data, input$mainInput)
  ...
  output$taxa_select_cus = renderUI({
    ...
    choice$fullName <- as.factor(choice$fullName)
    selectInput("taxa_select_cus", h5("Choose (super)taxon of interest:"),
                as.list(levels(choice$fullName)),
                levels(choice$fullName)[1])
  })
  ...
  output$selected_plot <- renderPlot({
    if(input$auto_update_selected == FALSE){
      ...
      # is changed
      isolate({
        selected_plot(input$inSeq,input$inTaxa,
                      input$apply_cluster,
                      ...
                      input$x_angle_select,
                      input$auto_update_selected,
                      input$plotCustom,
                      input$percent,
                      input$var1,
                      input$var2,
  ...
  output$selected_download <- downloadHandler(
    ...
    content = function(file) {
      ggsave(file, plot = selected_plot(input$inSeq,input$inTaxa,
                                        input$apply_cluster,
                                        input$x_axis_selected,
                                        input$var1_id,
                                        input$var2_id,
  ...
  output$point_info_status <- reactive({
    if(input$tabs == "Main profile"){
      print("point_info_status start")


      info <- mainpoint_info(v, input$rank_select, input$in_select, input$percent,
                             ...
                             input$inputTree,
                             input$x_axis,
                             input$selected)  # info = groupID,orthoID,supertaxon,mVar1,%spec,var2
      print("point_info_status end")
  ...
  output$detail_plot <- renderPlot({
    p <- detail_plot(input$var1_id, input$var2_id,
                     input$detailed_text,
                     v,
                     input$tabs,
                     input$var1_aggregate_by,
  ...
  output$download_detailed <- downloadHandler(
    ...
    content = function(file) {
      g <- detail_plot(input$var1_id, input$var2_id,
                       input$detailed_text,
                       v,
                       input$tabs,
                       input$var1_aggregate_by,
  ...
  point_infoDetail <- reactive({
    selDf <- detail_plotDt(v, input$tabs,
                           input$var1_aggregate_by, input$var2_aggregate_by,
                           input$end_index,
                           input$selected,
                           input$listIn,
  ...
  output$fasta <- renderText({
    ...
    if(is.null(info)){return()}
      ...
      data <- get_data_filtered(input$var1_aggregate_by, input$var2_aggregate_by,
                                input$end_index,
                                input$selected,
                                input$listIn,
                                input$st_index,
  ...
  getDomainFile <- reactive({
    ...
    if(input$demo_data == "demo" | input$demo_data == "ampk-tor"){
      ...
      if(input$anno_choose == "from file"){
        fileDomain <- input$fileDomain_input
  ...
  output$archi_download <- downloadHandler(
    ...
    content = function(file) {
      g <- archi_plot(input$demo_data,input$oneSeqFasta, input$label_archi_size, input$title_archi_size)
      grid.draw(g)
  ...
  gene_ageDf <- reactive({
    ...
    Dt <- get_taxa_list(FALSE, input$demo_data, input$mainInput)
    ...
    ### get main input data
    mdData <- droplevels(get_data_filtered(input$var1_aggregate_by, input$var2_aggregate_by,
                                           input$end_index,
                                           input$selected,
                                           input$listIn,
                                           input$st_index,
  ...
  output$gene_agePlot <- renderPlot({
    if(input$auto_update == FALSE){
      ...
      # is changed
      isolate({
        gene_agePlot(input$gene_age_text)
      })
    } else {
      gene_agePlot(input$gene_age_text)
    }
  ...
  output$gene_age_plot_download <- downloadHandler(
    ...
    content = function(file) { 
      ggsave(file, plot = gene_agePlot(input$gene_age_text),
  ...
  observe({
    if(input$add_cluster_cutom_profile == TRUE | input$add_cons_gene_custom_profile == TRUE | input$add_gc_genes_custom_profile == TRUE ){
  ...
  cons_geneDf <- reactive({
    ...
    ### get main input data
    mdData <- get_data_filtered(input$var1_aggregate_by, input$var2_aggregate_by,
                                input$end_index,
                                input$selected,
                                input$listIn,
                                input$st_index,
  ...
  # check if add_cluster_cutom_profile (profile clustering) -------------------
  # or add_custom_profile (gene age plot) are being clicked
  observe({
  ...
  download_data <- reactive({
    ...
    # filtered data
    dataOut <- get_data_filtered(input$var1_aggregate_by, input$var2_aggregate_by,
                                 v,
                                 input$end_index,
                                 input$selected,
                                 input$listIn,
    ...
    dataOut <- as.data.frame(dataOut[dataOut$var1 >= input$var1[1]
                                     & dataOut$var1 <= input$var1[2], ])

    if (!all(is.na(dataOut$var2))){
      dataOut <- as.data.frame(dataOut[dataOut$var2 >= input$var2[1]
                                       & dataOut$var2 <= input$var2[2], ])
    } else {
      dataOut$var2 <- 0
    }
    ...
    if (input$get_representative_main == TRUE){
      if (is.null(input$ref_var_main)) return()
        ...
        if (input$ref_var_main == input$var1_id){
          dataOut <- {
            dataOutRepresentative[dataOutRepresentative$var1 == dataOutRepresentative$var_best, ]
          }
        } else if (input$ref_var_main == input$var2_id){
          dataOut <- {
            dataOutRepresentative[dataOutRepresentative$var2 == dataOutRepresentative$var_best, ]
          }
        } else {
          dataOut <- dataOut
        }
    ...
    dataOut$ncbiID <- substr(dataOut$ncbiID,
                             5,
                             nchar(as.character(dataOut$ncbiID)))
  ...
  output$taxa_list_gc <- renderUI({
    ...
    if (v$doPlot == FALSE){
      ...
      # When the taxonomy is the same as in input and settings ----------------
        ...
        dt <- get_taxa_list(FALSE, input$demo_data, input$mainInput) # get the taxa
  ...
  output$plots_gc <- renderUI(get_plots_gc(as.character(input$selected_gene_gc),
                                           input$plot_gc,
                                           significant_genes_gc,
                                           input$show_p_value,
                                           input$highlight_significant,
                                           input$significance,
                                           input$var1_id,
                                           input$var2_id,
                                           input$x_size_gc,
  ...
  # variable to check the input type in the UI --------------------------------
  output$input_type <- reactive({
    filein <- input$mainInput
    if (is.null(filein)) return()
