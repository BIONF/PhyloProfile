...
shinyServer(function(input, output, session) {
  ...
  observe({
    if (!file.exists(isolate("data/taxonNamesFull.txt"))) {
      if (has_internet() == TRUE) {
        ncol <- max(count.fields("https://raw.githubusercontent.com/BIONF/phyloprofile-data/master/taxonNamesFull.txt",
                                 sep = "\t"))
        df <- read.table("https://raw.githubusercontent.com/BIONF/phyloprofile-data/master/taxonNamesFull.txt",
                         sep = "\t",
                         quote = "",
                         header = FALSE,
                         fill = TRUE,
