...
## OrthoXML
*PhyloProfile* is capable of reading in files in the *standardized OrthoXML format*, as e.g. generated by *Inparanoid* or *OMA Standalone*. *PhyloProfile* expects the *NCBI taxonomy IDs* to be present in the `species` tag in the XML as `NCBITaxId` like this:

`<species name="Dipodomys ordii" NCBITaxId="10020">`

### OMA Standalone
By default, the output of *OMA Standalone* does not include the correct `NCBITaxId` (c.f. `data/demo/oma_example.orthoxml`) but rather gives these as `<species name="Dipodomys_ordii" NCBITaxId="-1">`. With `scripts/convert_oma_standalone_orthoxml.py` we provide a basic Python script to enable the use of *OMA Standalone*.
